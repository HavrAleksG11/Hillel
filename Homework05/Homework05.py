...
need_year = search_criteria[0]
need_volume = search_criteria[1]
need_price = search_criteria[2]

podhodyat_brand = []

for brand in car_data:
    vse_dannie = car_data[brand]
    ...
    if year >= need_year:
        if volume >= need_volume:
            if price <= need_price:
                podhodyat_brand.append(brand)

n = len(podhodyat_brand)
for i in range(n):
    for j in range(0, n - i - 1):

        brand1 = podhodyat_brand[j]
        brand2 = podhodyat_brand[j + 1]

        price1 = car_data[brand1][4]
        price2 = car_data[brand2][4]

        if price1 > price2:
            temp = podhodyat_brand[j]
            podhodyat_brand[j] = podhodyat_brand[j + 1]
            podhodyat_brand[j + 1] = temp

kolichestvo = 0
for brand in podhodyat_brand:
    if kolichestvo < 5:
        inf = car_data[brand]

        print("Машина:", brand, "Рік:", inf[1], "Мотор:", inf[2], "Ціна:", inf[4])
